...
for(iteration in 1:1e4){
  ...
  Sigmainv.array1 <- rWishart(1, df = neu_p, Sigma = omega_p)
  Sigmainv.array2 <- rWishart(1, df = neu_p, Sigma = omega_p)
  
  Ones = matrix(1, nrow = nstocks, ncol = 1);
  Num_val <- (Sigmainv.array1[,,1] %*% Ones) * (sum(rowSums(Sigmainv.array1[,,1])));
  Den_val <- ((sum(rowSums(Sigmainv.array2[,,1])))^2);
